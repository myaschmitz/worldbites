...
// Build a lookup from country slug id → ISO alpha-3 mapId
const SLUG_TO_ALPHA3: Record<string, string> = Object.fromEntries(
  countries.map((c) => [c.id, c.mapId]),
);
...
export default function WorldMap({
  ...
  const currentNumericId = currentMapId
    ? (ALPHA3_TO_NUMERIC[currentMapId] ?? null)
    : null;

  function getGeoInfo(geo: GeoData): {
    fill: string;
    hoverFill: string;
    tooltipText: string;
  } {
    ...
    const completed = alpha3
      ? completedCountries.find((c) => SLUG_TO_ALPHA3[c.countryId] === alpha3)
      : undefined;
  ...
  return (
    <div className="w-full overflow-hidden rounded-xl bg-slate-900">
      <ComposableMap className="w-full" projectionConfig={{ scale: 147 }}>
        <ZoomableGroup>
        ...
        {hoveredInfo ? (
          ...
          <>
            <span className="flex items-center gap-1">
              <span className="inline-block h-2 w-2 rounded-sm bg-lime-500" />
              Completed
            </span>
            <span className="flex items-center gap-1">
